...
### Type Checking — mypy: 109 errors → 0

- **`db/models.py`** — Added `Mapped[T]` annotations on all bare `Column(SQLEnum(...))` fields (`role`, `status`, `current_stage`, `stage_type`, `agent_domain`, `agent_level`, `artifact_type`, `required_role`, `action`)
- **`agents/orchestrator.py`** — Fixed missing `return` at end of `_call_claude` retry loop; added `isinstance(block, TextBlock)` guard before `.text` access
- **`agents/pipeline_engine.py`** — Replaced `.awaitable_attrs.project` with direct attribute access after `selectinload`; cast `stage.stage_type` → `StageType(...)` and `pipeline.status` → `PipelineStatus(...)` where used as dict keys; renamed `send_rejection_alert` / `send_failure_alert` to the existing `notify_pipeline_failed`
- **`services/workspace_service.py`** — Method named `list` shadowed Python's builtin `list` type; added `from typing import List` and annotated return as `List[Workspace]`
- **`services/auth_service.py`** / **`api/v1/auth.py`** — Cast `user.hashed_password` to `str(...)` before passing to `verify_password`
- **`core/metrics.py`** — Wrapped all `.labels(...)` calls in `if metric is not None:` guards
- **`api/v1/routes.py`** — Fixed `dict[str, str]` return type mismatch (`"expires_in": 3600` → `"3600"`)
- **`workers/pipeline_worker.py`** — Replaced untyped `add_done_callback` lambdas with named typed callbacks
- **`api/v1/websocket.py`** — Added `queue: asyncio.Queue[str]` annotation
- **`api/v1/pipelines.py`** — Fixed implicit `Optional` on `user_id: UUID = None` → `UUID | None = None`
- **`main.py`** — Removed invalid `requests_per_minute` kwarg from `add_middleware`

### Tests — pytest: 33 failures → 0

- **`core/security.py`** — `verify_password(None, hashed)` raised `TypeError`; widened signature to `str | None` with early `return False`
- **`services/auth_service.py`** — `create_api_key` crashed on `api_key.created_at.isoformat()` when mock `refresh()` didn't populate DB defaults; added `or datetime.now(UTC)` fallback
- **`middleware/logging.py`** — Suppressed `httpx` / `httpcore` INFO logs leaking into `forge.access` caplog assertions
- **`main.py`** — Registered `v1_router` before auth-required resource routers so unit tests hit open stub routes instead of returning 401
