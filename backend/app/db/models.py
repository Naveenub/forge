"""
Database Models - Complete schema for Forge
...
class Project(Base):
    ...
    target_cloud = Column(String(50), nullable=True)  # aws, gcp, azure, on-prem
...
class PipelineStage(Base):
    ...
    started_at = Column(DateTime, nullable=True)
    completed_at = Column(DateTime, nullable=True)
    agent_output = Column(JSONB, nullable=True)  # Agent's generated content
    review_output = Column(JSONB, nullable=True)  # Review findings
    approval_output = Column(JSONB, nullable=True)  # Approval decision
...
class Artifact(Base):
    ...
    name = Column(String(255), nullable=False)
    content = Column(Text, nullable=True)  # For text artifacts
    file_path = Column(String(500), nullable=True)  # For binary artifacts
    checksum = Column(String(64), nullable=True)  # SHA-256
    version = Column(Integer, default=1)
    is_immutable = Column(Boolean, default=False)  # Locked after approval
    metadata = Column(JSONB, default={})
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class AuditLog(Base):
    ...
    # Audit logs are append-only (enforced at application level)
    __table_args__ = (
